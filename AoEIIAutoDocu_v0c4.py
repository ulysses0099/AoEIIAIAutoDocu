...
import os, sys, glob, copy, shutil, datetime
...
def PrintToLog(fp, level, task, message, suffix):
    """ prints log to file """
    log = level + ': ' + task + ' --- ' + message + suffix
    # print(log)
    fp.write(log)
...
def AddToResources(Resources, linetype, prevlinetype, Counter, content):
    ...
    PrintToLog(logfile, 'INFO', 'Resources', Resources[Counter['Module']][0][-1] + ' | ' + Resources[Counter['Module']][1][-1], '\n')
    return Resources
...
def PrepareLineFromResources(list_Resources):
    ...
    message = 'Restype: ' + Restype + ' | newline: \n\t' + newline
    PrintToLog(logfile, '\nINFO', 'PrepareLineFromResources', message, '\n')
...
mainfilename = Options['AI'] + '.per'
PrintMessage('Start Reading Main AI file: ' + mainfilename)
mainfile = open(mainfilename, 'r', encoding='utf8', errors='replace')
...
# ==============================================================================
# Initialize Logging
PrintMessage('Initialize File for Logging:')
logpath = Options['Docpath'] + os.sep + Options['Logfilename']

PrintSubMessage('Logfile: ' + logpath)

logfile = open(logpath , 'w')

PrintToLog(logfile, 'INFO', 'Program', 'Started', '\n')
PrintToLog(logfile, 'INFO', 'Initialization', 'AoEIIAutoDocu for ' + Options['AI'] + '.ai', '\n')
now = datetime.datetime.now()
PrintToLog(logfile, 'INGO', 'Date and Time', str(now), '\n')
...
pathtofile = Options['AI'] + '.per'
PrintToLog(logfile, 'INFO', 'Analysing AI File:', pathtofile, '\n')

Counter['Module'] = 0

Modules[0] = [0, [ai + '.per'], ['']]

with open(pathtofile, 'r', encoding='utf8', errors='replace') as file:
    ...
    for line in file.readlines():
        ...
        PrintToLog(logfile, 'INFO', 'linetype', linetype, '\n')
        PrintToLog(logfile, 'INFO', 'content', content, '\n')
...
for iStr in Structure.keys():
    ...
    for filename in Structure[iStr][2]:
        ...
        PrintToLog(logfile, 'INFO', 'Analysing AI File:', filename, '\n')
...
PrintToLog(logfile, 'INFO', 'SortedModules', 'Sort Modules into loading order', '\n')
...
message = 'moduleid: ' + str(SortedModules[0][0]) + ' | cModule: ' + SortedModules[0][1] + ' | Level: ' + str(SortedModules[0][2]) + ' | ResourceId: ' + str(SortedModules[0][3]) + ' | File:' + str(SortedModules[0][4])
PrintToLog(logfile, 'INFO', 'SortedModules', message, '\n')
...
while imod < nmod:
    ...
    message = 'moduleid: ' + str(SortedModules[cmod][0]) + ' | cModule: ' + SortedModules[cmod][1] + ' | Level: ' + str(SortedModules[cmod][2]) + ' | ResourceId: ' + str(SortedModules[cmod][3]) + ' | File:' + str(SortedModules[cmod][4])
    PrintToLog(logfile, 'INFO', 'SortedModules', message, '\n')
...
PrintToLog(logfile, 'INFO', 'HTML Output', 'Creating HTML Output Files', '\n')


htmlfilename = Options['Mainfolder'] + os.sep + Options['Docfolder'] + os.sep + ai + '.html'
PrintToLog(logfile, 'INFO', 'HTML Output', 'htmlfilename: ' + htmlfilename, '\n')
...
with open(htmlfilename, 'w') as htmlfile:
    ...
    for imod in range(0,len(SortedModules)):
        ...
        message = 'link to file: ' + link
        PrintToLog(logfile, 'INFO', 'HTML output', message, '\n')
    ...
    PrintToLog(logfile, 'INFO', 'HTML output', 'Add Statistics', '\n')
    ...
    PrintToLog(logfile, 'INFO', 'HTML output', 'Add Load and Module Structure', '\n')
    ...
    PrintToLog(logfile, 'INFO', 'HTML output', 'Add Folder and File Structure', '\n')
...
for moduleid in range(1, len(Modules)):
    ...
    with open(htmlfilename, 'w') as htmlfile:
        ...
        for imod in range(0,len(SortedModules)):
            ...
            PrintToLog(logfile, 'INFO', 'HTML output', 'link: ' + link, '\n')
            # print('link = ' + link)
            prevlevel = level
            level = SortedModules[imod][2]
            name = SortedModules[imod][4]
...
# ==============================================================================
PrintHead('Finished')
PrintToLog(logfile, 'INFO', 'Program', 'Finished', '\n')
logfile.close
